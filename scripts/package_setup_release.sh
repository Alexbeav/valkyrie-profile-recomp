...
#
# Writes: dist/valkyrie-profile-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
if [[ -f "${ROOT}/disc_set.json" ]]; then
  EXTRA_PROJECT+=(--project-file disc_set.json)
fi
if [[ -f "${ROOT}/disc_probe.json" ]]; then
  EXTRA_PROJECT+=(--project-file disc_probe.json)
fi
if [[ -f "${ROOT}/disc_probe.2.json" ]]; then
  EXTRA_PROJECT+=(--project-file disc_probe.2.json)
fi
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix valkyrie-profile-recomp \
  --exe-name Valkyrie_Profile_Recompiled \
  --display-name "Valkyrie Profile Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Valkyrie Profile disc" \
